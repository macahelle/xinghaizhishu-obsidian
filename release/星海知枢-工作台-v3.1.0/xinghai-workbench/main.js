...
const CONSTELLATION = [
  ...
  { label: "10-主题知识", path: "10-主题知识", x: 27, y: 23, orbit: true },
...
class DailyNoteConfirmModal extends Modal {
  ...
  onOpen() {
    ...
    const actions = element(contentEl, "div", "xh-modal-actions");
    const cancel = element(actions, "button", "", "仅查看日期");
    cancel.addEventListener("click", () => this.close());
...
class XinghaiWorkbenchView extends ItemView {
  ...
  renderConstellation(root) {
    ...
    CONSTELLATION.forEach((node) => {
      const button = element(hero, "button", `xh-star-node${node.center ? " is-center" : ""}${node.small ? " is-small" : ""}${node.orbit ? " has-orbit" : ""}`);
      ...
      if (node.orbit) {
        orb.style.outline = "1px solid rgba(224, 220, 245, 0.86)";
        orb.style.outlineOffset = "14px";
      }
  ...
  drawConstellation(hero, canvas) {
    ...
    context.shadowColor = "rgba(181, 174, 255, 0.3)";
