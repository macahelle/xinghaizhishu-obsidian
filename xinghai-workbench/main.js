...
class XinghaiWorkbenchView extends ItemView {
  ...
  async onOpen() {
    ...
    this.registerEvent(this.plugin.app.workspace.on("layout-change", () => {
      window.requestAnimationFrame(() => this.syncSidebarToggles());
    }));
    this.clockTimer = window.setInterval(() => this.updateLiveElements(), 1000);
    [0, 160, 600].forEach((delay) => window.setTimeout(() => this.syncSidebarToggles(), delay));
  }
  ...
  syncSidebarToggles() {
    this.sidebarToggleSyncs.forEach((sync) => sync());
  }
  ...
  renderConstellation(root) {
    ...
    this.sidebarToggleSyncs = [];
    const addSidebarToggle = (side) => {
      const button = element(topbar, "button", `xh-sidebar-toggle is-${side}`);
      const icon = element(button, "span", "xh-sidebar-toggle-icon");
      const sync = (collapsedOverride = null) => {
        const split = side === "left" ? this.plugin.app.workspace.leftSplit : this.plugin.app.workspace.rightSplit;
        const collapsed = collapsedOverride === null ? Boolean(split?.collapsed) : collapsedOverride;
        const sideLabel = side === "left" ? "左侧栏" : "右侧栏";
        const actionLabel = collapsed ? "展开" : "收起";
        button.toggleClass("is-visible", collapsed);
        button.setAttr("aria-label", `${actionLabel}${sideLabel}`);
        button.setAttr("title", `${actionLabel}${sideLabel}`);
        setIcon(icon, side === "left"
          ? collapsed ? "panel-left-open" : "panel-left-close"
          : collapsed ? "panel-right-open" : "panel-right-close");
      };
      this.sidebarToggleSyncs.push(sync);
      sync();
      button.addEventListener("click", () => {
        ...
        sync(false);
        [160, 480].forEach((delay) => window.setTimeout(() => sync(), delay));
      });
    };
    addSidebarToggle("left");
    addSidebarToggle("right");
